...
class TenderVersion(Base):
    ...
    serviceType: Mapped[TenderServiceTypeEnum] = mapped_column(
        Enum(TenderServiceTypeEnum), nullable=False
    )
...
class Tender(Base):
    ...
    organizationId: Mapped[UUID] = mapped_column(
    ...
    version_id: Mapped[UUID] = mapped_column(
        ForeignKey("tender_version.id", ondelete="CASCADE"), nullable=False
    )
...
class TenderInfo(Base):
    ...
    tender_version_id: Mapped[UUID] = mapped_column(
        ForeignKey("tender_version.id", ondelete="CASCADE"), primary_key=True
    )
